...
def assert_session_logon(
        rc, stdout, stderr, env_session_id, sf_session_id, result_sf_str,
        hmc_definition,  # noqa: F811
        session_name, exp_rc, exp_err, pdb_):
    ...
    if rc == 0:
        ...
        # If a valid session ID was provided in the env vars, verify that it is
        # still valid and that it has not been reused for the new session ID.
        if env_session_id:
            assert is_valid_hmc_session(hmc_definition, env_session_id)
            assert result_session_id != env_session_id

        # If a valid session ID was provided in the initial session file,
...
def test_session_logon(
    ...
    try:
        ...
        assert_session_logon(
            rc, stdout, stderr, env_session_id, sf_session_id, result_sf_str,
            hmc_definition, session_name, exp_rc, exp_err, pdb_)
...
def test_session_delete(
    ...
    try:
        ...
        # The code to be tested
        rc, stdout, stderr, _ = run_zhmc(zhmc_args, pdb_=pdb_, log=log)
