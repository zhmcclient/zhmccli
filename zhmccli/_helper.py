...
class CmdContext:
    ...
    def __init__(self, host, userid, password, no_verify, ca_certs,
                 ...
                 timestats, session_id, get_password, pdb):
        self._host = host
        self._userid = userid
    ...
    def execute_cmd(self, cmd, logoff=True, ignore_sessions=False):
        """
        Execute the command.

        Parameters:
          logoff (bool): Log off at the end of the command.
          ignore_sessions (bool): Execute the command without any session setup.
        """
        if self._session is None and not ignore_sessions:
