...
clobber: clean
	...
	rm -rf $(doc_build_dir) htmlcov htmlcov.end2end s.tox
...
$(done_dir)/ruff_$(pymn)_$(PACKAGE_LEVEL).done: Makefile $(done_dir)/develop_$(pymn)_$(PACKAGE_LEVEL).done
...
test: $(package_py_files) $(test_unit_py_files) $(test_function_py_files)
	PYTHONPATH=. coverage run --append -m pytest $(pytest_general_opts) $(pytest_test_opts) $(test_dir)/unit $(test_dir)/function
	coverage html
	@echo "Makefile: $@ done."
...
end2end: $(done_dir)/develop_$(pymn)_$(PACKAGE_LEVEL).done $(package_py_files) $(test_end2end_py_files)
	...
	TESTEND2END_LOAD=true PYTHONPATH=. coverage run --append -m pytest -v $(pytest_general_opts) $(pytest_test_opts) $(test_dir)/end2end
	coverage html
	@echo "Makefile: $@ done."
